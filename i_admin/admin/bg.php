...
if(isset($_FILES['video']['name']) and $_FILES['video']['error']==0){
	if($_FILES['video']['size']<=500000000){
		...
		$extenfc=array('mp4', 'avi', 'wmv', 'AVI', 'MP4', 'MP3', 'mpg', 'MPG', 'VOB', 'FLV', 'WMV', 'SWF', 'vob');
...
$req=$data->prepare('INSERT INTO videos VALUES(:video)');
$req->execute(array( 'video'=>$_POST['video'])); //or die (print_r($req->errorInfo()));
if($req){
	echo "Enregistrement termine";
}
else echo "Rien vraiment";
}
}
else echo "sjdbdbjhbhjdb";
?>